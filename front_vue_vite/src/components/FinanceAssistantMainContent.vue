<template>
    <div class="main-content">
        <div class="chat-container finance-chat" ref="chatContainer">
                                ...
                                <div class="collapsible-content" :class="message.detailsExpanded ? 'visible' : 'hidden'">
                                    <template v-if="message.outputParts && message.outputParts.length > 0">
                                        <template v-for="(part, partIndex) in message.outputParts" :key="partIndex">
                                            ...
                                            <div v-if="part.type === 'tool_execution' && part.process" class="tool-execution-section">
                                                <div class="tool-execution-title">工具执行过程</div>
                                                <div class="tool-execution-content"><pre>{{ part.process }}</pre></div>
                                            </div>
                                        </template>
                                    </template>
                                    <template v-else>
                                        ...
                                        <div v-if="getSearchSources(message).length > 0" class="search-sources-section">
                                            <div class="search-sources-title">搜索来源</div>
                                            <div class="search-sources-list">
                                        ...
                                        <div v-if="getToolExecutionProcess(message)" class="tool-execution-section">
                                            <div class="tool-execution-title">工具执行过程</div>
                                            <div class="tool-execution-content"><pre>{{ getToolExecutionProcess(message) }}</pre></div>
                                        </div>
                                    </template>
                                </div>
                            </template>
                ...
                <div class="input-buttons">
                    <div class="feature-buttons">
                        ...
                        <button 
                            ...
                            @click="$emit('toggle-feature', 'webSearch')"
                        >
                            联网搜索
                        </button>
...
const executeWithParams = (index, message) => {
    ...
    console.log('executeWithParams request:', JSON.stringify(request, null, 2));
...
const hasDetails = (message) => {
    if (message.outputParts && message.outputParts.length > 0) {
        return message.outputParts.some(part => 
            ...
            (part.type === 'tool_execution' && part.process)
        );
    }
    if (message.details) {
        ...
        if (Array.isArray(detailsArray)) {
            return detailsArray.some(item => 
                ...
                (item.type === 'tool_execution' && item.process)
            );
        }
...
const getToolExecutionProcess = (message) => {
    if (message.details) {
        let detailsArray = message.details;
        if (typeof message.details === 'string') {
            try {
        ...
        if (Array.isArray(detailsArray)) {
            const toolItem = detailsArray.find(item => item.type === 'tool_execution');
            return toolItem?.process || null;
        }
    }
    return null;
};
...
const handleSend = () => {
    ...
    const modelSelect = document.getElementById('financeModelSelect');
